...
load(paste0(here::here(), "/data/original/LACI_records.RData"))
...
# Graph -------------------------------------------------------------------
LACI_df <- readRDS(paste0(here::here(), "/data/processed/LACI_records.RDS"))
...
map <- ggplot(LACI_df, aes(decimalLongitude, decimalLatitude, group = month)) +
  geom_polygon(
    aes(long, lat, group = group), fill = 'grey', colour = "black", data = wmap)+
  stat_density_2d(aes(fill = ..level..), geom="polygon", alpha = 0.25)+
  geom_point(size=0.1)+
  coord_quickmap(xlim=c(-130,-40), ylim=c(-60, 60) )+
  labs(x=element_blank(), y = element_blank(), 
       title = 'HOARY Bat Records',
       fill = element_blank())+
...
map+
  facet_wrap(~Fmonth)+
  ggsave(file=paste0(here::here(), "/output/LACI_month.jpg"), width = 8, height = 10.5)
